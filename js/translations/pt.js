// ============================================
// 🇵🇹 PORTUGUÊS - DOVE OF HOPE
// ============================================

const pt = {
  code: 'pt',
  name: 'Português',
  flag: '🇵🇹',
  
  ui: {
    score: 'Pontos',
    clouds: 'Nuvens',
    level: 'Capítulo',
    lives: 'Vida',
    language: 'Idioma',
    choose_language: 'Escolher idioma',
    language_changed: 'Idioma alterado',
    install: 'INSTALAR',
    tap_share: 'Toque em',
    add_home: 'Adicionar à tela inicial',
    confirm: 'Confirmar',
    install_app: 'Instalar aplicativo',
    install_desktop: 'Abrir no Chrome/Edge para instalar',
    close: 'Fechar',
    pause: 'Pausar',
    music: 'Música',
    sfx: 'Efeitos',
    menu: 'Menu'
  },
  
  export: {
    score_label: 'Pontos',
    level_label: 'Nível',
    courage_message: 'Coragem, herói valente!',
    divine_light: 'em luz divina'
  },
  
  bonus: {
    completed_title: '🎉 BÔNUS COMPLETO',
    items_collected: 'Itens coletados',
    bonus_score: 'PONTOS BÔNUS'
  },
  
  buttons: {
    start: 'Acenda!',
    play: 'JOGAR',
    restart: 'REJOGAR',
    infinite_mode: 'Modo Infinito',
    close: 'Fechar',
    start_bonus: 'Começar'
  },
  
  messages: {
    start: 'Voa, pequena pomba! Dissipa as nuvens!',
    restart: 'A pomba renasce! Vamos de novo!',
    gameOver: 'Game Over! Mas na adversidade, brilhas ainda mais!',
    victory: 'VITÓRIA! Salmo 99.1 - A vida é uma aventura extraordinária! - diz Orígenes',
    levelUp: 'Nível {level} - A luz cresce!',
    motivation: 'É hora de acender a luz - É um bom momento para realizar seus sonhos!',
    welcome: 'Bem-vindo, Cadete Primeiro Voo!'
  },

  profile: {
    nickname: 'Seu apelido',
    email: 'Email (opcional)',
    enter_nickname: 'Digite seu apelido...',
    enter_email: 'seu@email.com'
  },
  
  gameplay: [
    'Mexe-te, é a hora!',
    'A pomba traz esperança!',
    'Continua, a luz retorna!',
    'Tuas asas carregam esperança!',
    'Persevera, o sol te espera!',
    'Voa para teus sonhos!',
    'Depois da chuva vem o sol!',
    'Saber dançar na chuva é saber ser feliz!',
    'Tua fé é mais forte que a tempestade!',
    'Dança com os elementos, és livre!'
  ],
  
  rain: [
    'Depois da chuva vem o sol!',
    'Tua fé é mais forte que a tempestade!',
    'A tempestade revela a força de tuas asas!'
  ],
  
  powerups: {
    try_again: 'Mas na adversidade, você brilha ainda mais!',
    umbrella: 'Guarda-chuva ativado! Dance com os elementos!',
    speed: 'Velocidade divina! Suas asas são livres!',
    multishot: 'Luz tripla! Seu coração brilha!',
    rainbow: 'Escudo Arco-íris ativado! Proteção divina!',
    star_collected: '⭐ Estrela coletada! ({count} em estoque) - Clique duplo para onda de choque!',
    level_up: '✨ Nível {level}: {title}',
    new_record: '🏆 NOVO RECORDE! Pontuação: {score}',
    new_record_popup: '🎉 Novo recorde!\n\nPontuação: {score}\n\nSua pontuação foi salva.\nClique em Exportar no menu Tesouro para compartilhá-la.',
    infinite_mode: '🔄 Modo Infinito ativado! Versículos a cada 100 nuvens ☁️',
    record_saved: '☁️ Recorde salvo!',
    heart: '💖 +1 Vida! A graça de Deus te protege!',
    boss_defeated: '🏆 CHEFE DERROTADO! +{score} pts',
    storm_cleared: '🌈 A tempestade se dissipa! A esperança brilha novamente!',
    continue: '✨ Continuar',
    save_system: '✨ Sistema de salvamento'
  },
  
  bonus: {
    collect_blessings: 'Colete as bênçãos:',
    star: 'Estrela',
    heart: 'Coração',
    cross: 'Cruz',
    rainbow: 'Arco-íris',
    coin_trap: 'Moeda = -200 pts (ARMADILHA!)',
    objective: 'Objetivo:',
    objects: 'objetos',
    level_title: 'NÍVEL BÔNUS'
  },
  
  treasure: {
    title: '💎 Menu Tesouro',
    my_records: '🏆 Meus Recordes',
    best_score: 'Melhor Pontuação',
    max_level: 'Nível Máximo',
    star_stock: '⭐ Estoque de Estrelas',
    chapters_conquered: '🏅 Capítulos Conquistados',
    chapters_completed: 'capítulos completados',
    infinite_progress: '♾️ Progresso Modo Infinito',
    verse_collection: 'Coleta de versículos aleatórios a cada 100 nuvens',
    xp_clouds: 'XP: {count} nuvens',
    next_verse: 'Próximo verso em: {count}',
    verses_collected: '📚 {count} versículos coletados',
    last_played: 'Última partida',
    creator: '✨ Criador',
    creator_name: '🎮 Emmanuel Payet',
    creator_bio: 'Desenvolvedor & Criador de jogos inspirados',
    audiomack: '📱 Audiomack',
    creator_message: 'Que este jogo te inspire e te fortifique! 🕊️',
    discover_artist: '🎵 Descobrir o Artista',
    cloud_connect: '☁️ Conectar-se à nuvem',
    cloud_manage: '☁️ Gerenciar minha conexão',
    life_tips_faith: '✝️ Fé',
    life_tips_courage: '💪 Coragem',
    life_tips_peace: '🕊️ Paz',
    life_tips_love: '❤️ Amor',
    life_tips_prompt: 'Clique numa aba para descobrir um conselho de vida',
    infinite_chapter: 'Capítulo {number}',
    my_profile: 'Meu Perfil',
    nickname: 'Apelido',
    email: 'E-mail',
    cloud_status: 'Nuvem',
    connected: 'Conectado',
    disconnected: 'Desconectado',
    highest_level: 'Melhor Pontuação',
    last_game: 'Última partida',
    testimonies_title: 'Testemunhos de Esperança',
    testimonies_description: 'Descubra histórias de coragem e cura',
    read_testimonies: 'Ler testemunhos',
    save_cloud: 'Salvar na Nuvem',
    share_score: 'Partilhar a minha pontuação',
    not_set: 'Não definido',
    boss_wisdom_title: 'Sabedoria dos Chefes',
    boss_wisdom_subtitle: 'Derrote os chefes para desbloquear os seus ensinamentos profundos',
    boss_wisdom_collected: 'sabedoria desbloqueada',
    boss_wisdom_locked: 'Esta sabedoria ainda está bloqueada. Derrote este chefe para desbloqueá-la!',
    boss_wisdom_philosophy: 'Filosofia e Ensinamento',
    boss_wisdom_victory: 'Mensagem de Vitória',
    boss_wisdom_close: 'Fechar'
  },
  
  intro: 'A vida é muitas vezes uma longa batalha, é preciso coragem e força para aguentar nos dias maus. Valente herói, levanta-te, e com o amor de Deus transforma as nuvens cinzentas em bem.',
  
  clouds_cleared: '{count} nuvens dissipadas',
  transform_clouds: 'Transforma as nuvens cinzentas',  level_titles: {
    1: 'Alvorada da Esperança',
    2: 'Sopro Divino',
    3: 'Asas de Fé',
    4: 'Perdão e Liberdade',
    5: 'Escolhido e Precioso',
    6: 'Força Interior',
    7: 'Tempestade de Graça',
    8: 'Amor Incondicional',
    9: 'Herói da Luz',
    10: 'Nova Criatura',
    11: 'Coragem do Leão',
    12: 'Porta da Sabedoria',
    13: 'Novo Céu',
    14: 'A Aventura Eterna'
  },
  

  
  levels: {
    1: 'Você deu seus primeiros passos na luz! Você não está sozinho nesta batalha.',
    2: 'Como uma águia, Deus te fortalece! Ele te escolhe como você é, com suas fraquezas e sua beleza.',
    3: 'Você não é inútil, você não é rejeitado. Você é infinitamente amado!',
    4: 'Seus erros não te definem. O amor de Deus apaga tudo e te torna livre!',
    5: 'Você é escolhido! Não por causa de sua perfeição, mas por pura graça.',
    6: 'Na sua fraqueza, Sua força se revela. Você pode tudo, pois Ele te fortalece!',
    7: 'Os desafios se multiplicam, mas você dança com os elementos!',
    8: 'Você não precisa ser perfeito para ser amado. Você já é perfeitamente amado!',
    9: 'Você não é pecador demais para Deus. Você é Seu herói, Seu filho amado!',
    10: 'O velho passou, eis o novo! Você renasce na luz divina.',
    11: 'Levanta-te com coragem! Você é digno, você é capaz, você é vitorioso!',
    12: 'A Bíblia não é um livro de julgamento, mas um tesouro de amor para você! Abra-a com confiança.',
    13: 'Você se aproxima do cume! Leia a Bíblia como um vencedor, não como um condenado. Você é escolhido e amado!',
    14: 'Último desafio antes da vitória total! A Bíblia te espera, ela foi escrita para você, herói da luz! Continue, você está quase no topo!'
  },
  
  victory_message: 'VITÓRIA! Você transformou as trevas em luz! Agora, abra sua Bíblia com confiança: você é escolhido, você é amado, você é vitorioso! "A vida é uma aventura extraordinária" - Salmo 99:1 diz Orígenes. Que a paz e o amor reinem em seu coração!',
  
  infinite_messages: [
    'Continue sua aventura! Cada nuvem dissipada é uma vitória!',
    'Você é um campeão da luz! Continue sua busca!',
    'O infinito pertence a você! Que sua fé cresça ainda mais!',
    'Você supera todos os limites! Bravo, herói eterno!',
    'Sua perseverança é admirável! Continue brilhando!'
  ],

  popups: {
    not_connected: '⚠️ Não conectado!\n\nConecte-se primeiro via o menu Tesouro.',
    no_data_to_save: '⚠️ Nenhum dado para salvar\n\nJogue primeiro para criar dados!',
    saving: '⏳ Salvando...',
    save_failed: '❌ Falha ao salvar!\n\n{error}',
    save_error: '❌ Erro de salvamento!\n\n{error}',
    data_imported: '✅ Dados importados com sucesso!',
    invalid_file: '❌ Arquivo inválido',
    enter_nickname_email: '❌ Por favor insira apelido E email!',
    invalid_email: '❌ Email inválido!',
    connected_local_saved: '✅ Conectado!\n\nDados locais salvos na nuvem.',
    connected_cloud_restored: '✅ Conectado!\n\nDados da nuvem restaurados.',
    connected_success: '✅ Conectado com sucesso!\n\nSeu progresso será salvo automaticamente online.',
    reset_confirm: '⚠️ Deseja realmente redefinir todos os seus dados?',
    data_conflict: '⚠️ Conflito de dados detectado!',
    connection_error: '❌ Erro de conexão'
  },

  testimony: {
    title: '💎 Tesouro de Vida - Emmanuel',
    paragraph1: 'Houve um tempo em que a doença me isolou terrivelmente e nuvens escuras vieram invadir minha vida. Tentei desistir mas antes disso clamei a Deus! <strong>É a ti que dou minha vida e não às sombras traicoeiras.</strong>',
    paragraph2: 'Como uma oração, meu clamor foi ouvido e minha vida mudou desde o dia em que soube em meu coração que <em>sou amado como sou</em>. Um longo trabalho de confiança, coragem e mudança começou em mim.',
    paragraph3: 'Aprendi com os erros, aprendi a não olhar sempre para trás sem nunca esquecer. Mas acima de tudo aprendi a olhar para o futuro com paciência, esperança e alegria.',
    paragraph4: 'Uma alegria que encontrei no louvor e na adoração a Deus salvador, aquele que estendeu a mão quando ninguém podia estar lá para mim. Ele me tirou do vale da morte e me fez ver o que é verdadeiramente importante: <strong>a vida simples, ordinária</strong>.',
    paragraph5: 'Vi além dos meus desejos materialistas, vi que devia olhar para o que tenho e não para o que não tenho. E amar o que sou, como quero ser. <em>Vi a esperança cara a cara.</em>',
    paragraph6: 'Pedi e recebi respostas inefáveis que produziram em mim uma fé inabalável. Através das dificuldades cresci e não me arrependo de ter perseverado, esperado mais e mais porque <strong>cada colina escondia tesouros de vida</strong>, cada montanha conquistada pela coragem de mover trouxe um futuro melhor, memórias de vida extraordinária, encontros e lugares maravilhosos.',
    final_message: '🕊️ É por isso que Jesus te diz: Nunca desista, persevere até o impossível! 🕊️'
  },

  life_tips: {
    faith: {
      title: '✝️ Conselhos de Fé',
      tip1: '💫 "A fé é a certeza daquilo que esperamos" - Hebreus 11:1',
      tip2: '🙏 Ore todos os dias, mesmo alguns minutos são suficientes para fortalecer sua alma',
      tip3: '📖 Leia a Bíblia regularmente para nutrir sua fé e encontrar respostas',
      tip4: '✨ Confie em Deus nos momentos difíceis, Ele tem um plano para você',
      tip5: '🌟 Cerque-se de crentes que te encorajam na sua fé'
    },
    courage: {
      title: '💪 Conselhos de Coragem',
      tip1: '🦁 "Seja forte e corajoso, não tema" - Josué 1:9',
      tip2: '🔥 Enfrente seus medos um por um, cada pequena vitória conta',
      tip3: '🎯 Estabeleça objetivos claros e avance passo a passo em direção a eles',
      tip4: '👊 Nunca desista, mesmo quando for difícil - a perseverança compensa',
      tip5: '⭐ Lembre-se de seus sucessos passados para encontrar força para continuar'
    },
    peace: {
      title: '🕊️ Conselhos de Paz',
      tip1: '🌿 "Deixo-vos a paz, a minha paz vos dou" - João 14:27',
      tip2: '🧘 Tire um tempo todos os dias para descansar e meditar',
      tip3: '🌺 Perdoe aqueles que te feriram, liberte-se do peso do rancor',
      tip4: '🌈 Busque a paz nas pequenas coisas: a natureza, a música, a oração',
      tip5: '💕 Cerque-se de pessoas positivas que trazem serenidade'
    },
    love: {
      title: '❤️ Conselhos de Amor',
      tip1: '💖 "Amai-vos uns aos outros como eu vos amei" - João 13:34',
      tip2: '🤝 Mostre bondade para com todos, mesmo aqueles que são diferentes',
      tip3: '🎁 Dê sem esperar nada em troca, o amor verdadeiro é desinteressado',
      tip4: '👂 Ouça verdadeiramente os outros, esteja presente para aqueles que sofrem',
      tip5: '✨ Comece por se amar: você é precioso aos olhos de Deus'
    }
  },

  ranks: {
    1: {
      title: 'CADETE - Primeiros Voos',
      message: 'Sua jornada celeste começa. Assuma os comandos e aprenda a navegar entre as nuvens. Cada missão te aproxima do seu destino.'
    },
    2: {
      title: 'PILOTO APRENDIZ - Em Formação',
      message: 'Você progride rapidamente! Sua habilidade cresce, sua confiança se fortalece. Continue a se elevar em direção às alturas.'
    },
    3: {
      title: 'AVIADOR - Batismo do Céu',
      message: 'Suas asas se abrem com confiança! Você domina os fundamentos do voo e descobre a liberdade dos céus.'
    },
    4: {
      title: 'PILOTO CONFIRMADO - Navegador Experiente',
      message: 'Os ventos te conhecem, as nuvens te respeitam! Você navega com facilidade nas correntes celestes.'
    },
    5: {
      title: 'ÁS DOS CÉUS - Virtuoso Alado',
      message: 'Você dança entre as nuvens como um mestre! Seus reflexos se aguçam, sua precisão se torna lendária.'
    },
    6: {
      title: 'ESQUADRÃO DE ELITE - Força Aérea',
      message: 'Você faz parte dos melhores! Sua experiência inspira outros pilotos. Você voa com uma graça divina.'
    },
    7: {
      title: 'COMANDANTE - Tático Celestial',
      message: 'Missões estratégicas são confiadas a você! Você comanda o céu com sabedoria e coragem, guiado pela luz divina.'
    },
    8: {
      title: 'ESTRATEGISTA DAS NUVENS - Mestre Tático',
      message: 'Você antecipa cada movimento! Sua inteligência tática e visão estratégica superam o comum.'
    },
    9: {
      title: 'VETERANO - Forjado por Tempestades',
      message: 'Você atravessou mil tempestades! Cada cicatriz conta uma vitória. Você é uma lenda viva dos céus.'
    },
    10: {
      title: 'HERÓI CELESTIAL - Campeão Renomado',
      message: 'Seus feitos são cantados em todos os reinos! Você encarna a coragem e a determinação inabalável.'
    },
    11: {
      title: 'LENDA - Mito Encarnado',
      message: 'Seu nome ressoa em todos os céus! Os cadetes sonham em se tornar como você. Você encarna a excelência e a graça.'
    },
    12: {
      title: 'CAMPEÃO ETERNO - Glória Imortal',
      message: 'Sua fama atravessa as eras! Seu legado inspirará as futuras gerações de pilotos celestes.'
    },
    13: {
      title: 'MESTRE PILOTO - Sinfonia Aérea',
      message: 'Você não voa mais, você compõe uma sinfonia celeste! Cada movimento é perfeição, cada missão uma obra-prima.'
    },
    14: {
      title: 'ASA DO CÉU - Guardião Eterno',
      message: 'Você transcendeu a mortalidade! Guardião celestial, protetor dos céus, seu voo agora é eterno. O infinito te pertence!'
    }
  },

  verses: {
    level: {
      1: { text: 'A luz brilha nas trevas', reference: 'João 1:5' },
      2: { text: 'O Espírito de Deus pairava sobre as águas', reference: 'Gênesis 1:2' },
      3: { text: 'Os que esperam no Senhor terão asas como águias', reference: 'Isaías 40:31' },
      4: { text: 'Onde está o Espírito do Senhor, aí há liberdade', reference: '2 Coríntios 3:17' },
      5: { text: 'Chamei-te pelo teu nome, tu és meu', reference: 'Isaías 43:1' },
      6: { text: 'Tudo posso naquele que me fortalece', reference: 'Filipenses 4:13' },
      7: { text: 'Depois da chuva vem o sol', reference: 'Jó 37:11' },
      8: { text: 'O amor nunca falha', reference: '1 Coríntios 13:8' },
      9: { text: 'A tua fé te salvou', reference: 'Lucas 7:50' },
      10: { text: 'Se alguém está em Cristo, nova criatura é', reference: '2 Coríntios 5:17' },
      11: { text: 'Sede fortes e corajosos', reference: 'Josué 1:9' },
      12: { text: 'Tua palavra é lâmpada para os meus pés', reference: 'Salmo 119:105' },
      13: { text: 'Vós sois raça eleita', reference: '1 Pedro 2:9' },
      14: { text: 'A vida é uma aventura extraordinária', reference: 'Salmo 99:1 (diz Orígenes)' }
    },
    rank: {
      1: { text: 'De longe o Senhor me apareceu: "Eu te amei com amor eterno; por isso com benignidade te atraí."', reference: 'Jeremias 31:3' },
      2: { text: 'O Senhor dá força ao cansado e multiplica as forças do que não tem nenhum vigor.', reference: 'Isaías 40:29' },
      3: { text: 'Os que esperam no Senhor terão asas como águias', reference: 'Isaías 40:31' },
      4: { text: 'Onde está o Espírito do Senhor, aí há liberdade', reference: '2 Coríntios 3:17' },
      5: { text: 'Chamei-te pelo teu nome, tu és meu', reference: 'Isaías 43:1' },
      6: { text: 'Tudo posso naquele que me fortalece', reference: 'Filipenses 4:13' },
      7: { text: 'Porque Deus não é de confusão, mas de paz', reference: '1 Coríntios 14:33' },
      8: { text: 'Não temas, porque eu estou contigo', reference: 'Isaías 41:10' },
      9: { text: 'Sede fortes e corajosos', reference: 'Josué 1:9' },
      10: { text: 'A tua fé te salvou', reference: 'Lucas 7:50' },
      11: { text: 'Se alguém está em Cristo, nova criatura é', reference: '2 Coríntios 5:17' },
      12: { text: 'Vós sois raça eleita, sacerdócio real', reference: '1 Pedro 2:9' },
      13: { text: 'Depois da chuva vem o sol', reference: 'Jó 37:11' },
      14: { text: 'O amor nunca falha', reference: '1 Coríntios 13:8' }
    },
    infinite: [
      { text: 'Não se turbe o vosso coração', reference: 'João 14:1' },
      { text: 'Estou convosco todos os dias', reference: 'Mateus 28:20' },
      { text: 'O Senhor é a minha luz e a minha salvação', reference: 'Salmo 27:1' },
      { text: 'Vós sois a luz do mundo', reference: 'Mateus 5:14' },
      { text: 'O amor cobre multidão de pecados', reference: '1 Pedro 4:8' },
      { text: 'Porque Deus amou tanto o mundo', reference: 'João 3:16' },
      { text: 'Buscai primeiro o reino de Deus', reference: 'Mateus 6:33' },
      { text: 'O justo viverá pela fé', reference: 'Romanos 1:17' },
      { text: 'Tudo é possível ao que crê', reference: 'Marcos 9:23' },
      { text: 'Deus é amor', reference: '1 João 4:8' },
      { text: 'Nada nos poderá separar do amor de Deus', reference: 'Romanos 8:39' },
      { text: 'Andai no Espírito', reference: 'Gálatas 5:16' },
      { text: 'Regozijai-vos sempre no Senhor', reference: 'Filipenses 4:4' },
      { text: 'A paz de Deus guarda os vossos corações', reference: 'Filipenses 4:7' },
      { text: 'Não temas, porque eu estou contigo', reference: 'Isaías 41:10' },
      { text: 'O Senhor é a minha força e o meu escudo', reference: 'Salmo 28:7' },
      { text: 'Bem-aventurados os puros de coração', reference: 'Mateus 5:8' },
      { text: 'A esperança não decepciona', reference: 'Romanos 5:5' },
      { text: 'Fazei tudo com amor', reference: '1 Coríntios 16:14' },
      { text: 'A graça do Senhor seja convosco', reference: '1 Coríntios 16:23' }
    ]
  },

  bosses: {
    victory_title: '🏆 VITÓRIA 🏆',
    spiritual_victory: '🌟 Vitória Espiritual 🌟',
    spider: {
      name: '🕷️ A Aranha das Trevas',
      intro: 'Uma aranha gigante tece sua teia de medo... Mas "Deus não nos deu espírito de medo"!',
      phase_1: '🕸️ "Porque Deus não nos deu espírito de covardia, mas de poder, de amor e de moderação." (2 Timóteo 1:7)\n\nO MEDO DA DOR, o MEDO DE SER PICADO... Esses medos te paralisam!',
      phase_2: '✨ "Não temas, porque eu sou contigo; não te assombres, porque eu sou o teu Deus." (Isaías 41:10)\n\nMEDOS ILÓGICOS e FOBIAS não têm poder sobre aqueles que caminham na fé!',
      phase_3: '🛡️ "Ainda que eu ande pelo vale da sombra da morte, não temerei mal nenhum, porque tu estás comigo." (Salmo 23:4)\n\nO medo é um MECANISMO PROTETOR, mas não deve te DOMINAR!',
      defeated: '🦋 VITÓRIA SOBRE O MEDO!\n\n"O perfeito amor lança fora o medo." (1 João 4:18)\n\nVocê conquistou o medo paralisante! A fé transforma o medo em coragem.',
      wisdom: 'A aranha simboliza todos os nossos medos: o medo da dor, de ser picado, fobias ilógicas. Esses medos são frequentemente mecanismos protetores normais que nos mantêm a salvo do perigo. Mas quando se tornam excessivos, nos paralisam. A Bíblia diz: "Deus não nos deu espírito de covardia, mas de poder, amor e moderação." A fé não nega o perigo, mas nos dá a coragem para enfrentá-lo. Em Cristo, aprendemos a distinguir entre cautela saudável e medo paralisante. O perfeito amor de Deus lança fora o medo!'
    },
    flying_snake: {
      name: '🐍 A Serpente Voadora',
      intro: 'A antiga serpente, símbolo da tentação, ergue-se diante de ti... Mas "resisti ao diabo, e ele fugirá de vós"!',
      phase_1: '🔥 "Vigiai e orai, para que não entreis em tentação." (Mateus 26:41)\n\nTENTAÇÃO, a MENTIRA ORIGINAL, a DÚVIDA que sussurra: "É assim que Deus disse...?"',
      phase_2: '⚔️ "Resisti ao diabo, e ele fugirá de vós. Chegai-vos a Deus, e ele se chegará a vós." (Tiago 4:7-8)\n\nO veneno do COMPROMISSO e da DESOBEDIÊNCIA não pode alcançar aqueles que permanecem perto de Deus!',
      phase_3: '✝️ "E, como Moisés levantou a serpente no deserto, assim importa que o Filho do Homem seja levantado." (João 3:14)\n\nCristo ESMAGOU a cabeça da serpente! Sua vitória é a sua vitória!',
      defeated: '🌟 VITÓRIA SOBRE A TENTAÇÃO!\n\n"Esta te ferirá a cabeça, e tu lhe ferirás o calcanhar." (Gênesis 3:15)\n\nVocê conquistou a tentação! Em Cristo, a antiga serpente está derrotada.',
      wisdom: 'A serpente voadora representa a tentação e a mentira original do Jardim do Éden: "Certamente não morrereis... sereis como Deus." É a voz que semeia dúvida sobre a bondade de Deus, empurrando-nos para o compromisso e a desobediência. Mas a Bíblia ensina: "Resisti ao diabo, e ele fugirá." A tentação não é pecado - é o convite ao pecado. Jesus mesmo foi tentado, mas resistiu através da Palavra de Deus. Na Cruz, Cristo esmagou a cabeça da serpente. Através de sua vitória, temos o poder de resistir e vencer toda tentação. Chegai-vos a Deus, e Ele se chegará a nós!'
    },
    crazy_chicken: {
      name: '🐔 A Galinha Louca',
      intro: 'Uma galinha em pânico correndo em todas as direções... Medos falsos te deixam louco e te paralisam!',
      phase_1: '😰 "Porque Deus não nos deu espírito de covardia, mas de poder, de amor e de moderação." (2 Timóteo 1:7)\n\nFALSOS MEDOS SUBJETIVOS: "E se...?", "Talvez...", "Tenho medo que..." - Medos IMAGINÁRIOS que te IMPEDEM!',
      phase_2: '🧠 "Não andeis ansiosos de coisa alguma; em tudo, porém, sejam conhecidas, diante de Deus, as vossas petições, pela oração." (Filipenses 4:6)\n\nCENÁRIOS CATASTRÓFICOS na sua cabeça, MEDOS IRRACIONAIS que te deixam louco! PARE!',
      phase_3: '✨ "Porque Deus é quem efetua em vós tanto o querer como o realizar, segundo a sua boa vontade." (Filipenses 2:13)\n\nEsses medos SUBJETIVOS NÃO SÃO A REALIDADE! Deus te deu SABEDORIA, não LOUCURA!',
      defeated: '🦅 VITÓRIA SOBRE OS MEDOS FALSOS!\n\n"Não temas, porque eu sou contigo." (Isaías 41:10)\n\nVocê conquistou os medos imaginários! Agora você caminha na sabedoria, não na loucura.',
      wisdom: 'A galinha louca representa os falsos medos subjetivos que nos deixam loucos: "E se eu falhar?", "E se as pessoas me julgarem?", "E se eu não for bom o suficiente?". Esses medos imaginários, esses cenários catastróficos em nossas cabeças nos paralisam e nos impedem. Entramos em pânico como uma galinha assustada por coisas que talvez nunca aconteçam! A Bíblia diz: "Deus não nos deu espírito de covardia, mas de poder, amor e moderação." 95% dos nossos medos nunca se tornam realidade. Esses medos subjetivos não são a realidade - são mentiras do inimigo para nos impedir de avançar. "Não andeis ansiosos" - Deus nos chama a substituir medos falsos por fé, pânico por paz, cenários catastróficos por confiança em Sua providência.'
    },
    grim_reaper: {
      name: '💀 A Morte',
      intro: 'A própria morte se ergue diante de ti... Mas lembre-se: Cristo venceu a morte!',
      phase_1: '🕊️ "Onde está, ó morte, a tua vitória? Onde está, ó morte, o teu aguilhão?" (1 Coríntios 15:55)\n\nO medo da morte não tem poder sobre aqueles que creem na vida eterna!',
      phase_2: '✝️ "Disse-lhe Jesus: Eu sou a ressurreição e a vida. Quem crê em mim, ainda que morra, viverá." (João 11:25)\n\nA vida sempre triunfa sobre as trevas! Cada momento é uma VITÓRIA!',
      phase_3: '🌟 "Ensina-nos a contar os nossos dias, para que alcancemos coração sábio." (Salmo 90:12)\n\nNão importa quanto tempo resta - VIVA PLENAMENTE! Cada dia é um PRESENTE!',
      defeated: '🙏 VITÓRIA SOBRE A MORTE!\n\n"Agora, pois, já nenhuma condenação há para os que estão em Cristo Jesus." (Romanos 8:1)\n\nVocê conquistou o medo! A vida eterna é sua pela graça de Deus.',
      wisdom: 'A morte não é um fim, mas uma transformação. Através de Cristo, passamos da vida terrena para a vida eterna. Não importa quanto tempo nos resta ou como vivemos - o melhor é viver cada momento como uma vitória, cada novo ano como um presente. Mesmo na doença, mesmo nas provações, a ESPERANÇA e o OTIMISMO nos dão asas e tornam o impossível frequentemente possível! A Bíblia diz: "Ensina-nos a contar nossos dias, para que alcancemos um coração sábio." Cada dia é precioso. Cada respiração é graça. A fé cristã nos ensina que a morte foi derrotada na Cruz, e que a esperança da ressurreição ilumina nosso caminho. Vivamos, portanto, plenamente, com alegria e gratidão, porque "Este é o dia que o Senhor fez; regozijemo-nos e alegremo-nos nele!" (Salmo 118:24)'
    },
    wall: {
      name: '🧱 A Muralha de Correntes',
      intro: 'Um muro de medos e dúvidas se ergue diante de ti... Mas "onde está o Espírito do Senhor, aí há liberdade"!',
      phase_1: '⛓️ "Para a liberdade foi que Cristo nos libertou. Permanecei, pois, firmes e não vos submetais, de novo, a jugo de escravidão." (Gálatas 5:1)\n\nAs correntes do MEDO, da DÚVIDA e da VERGONHA não têm poder sobre você!',
      phase_2: '🔓 "Se, pois, o Filho vos libertar, verdadeiramente sereis livres." (João 8:36)\n\nCada corrente quebrada é uma vitória sobre as mentiras do inimigo!',
      phase_3: '✨ "Tudo posso naquele que me fortalece." (Filipenses 4:13)\n\nA verdade de Deus destrói as fortalezas e muralhas que te aprisionam!',
      defeated: '🎉 LIBERDADE EM CRISTO!\n\n"Conhecereis a verdade, e a verdade vos libertará." (João 8:32)\n\nVocê quebrou as correntes! Você é livre através do poder do amor de Deus.',
      wisdom: 'Os medos são como correntes invisíveis que nos impedem de avançar. A Bíblia nos ensina que "Deus não nos deu espírito de covardia, mas de poder, amor e moderação." Cada medo conquistado através da fé é uma corrente quebrada. Em Cristo, somos chamados a viver em liberdade, libertados das mentiras que nos mantêm cativos. A verdade nos liberta!'
    },
    dark_cloud: {
      name: '☁️ A Nuvem Escura',
      intro: 'Uma nuvem de tempestade ameaça engolir tudo... Mas "Deus é o nosso refúgio e fortaleza, socorro bem presente nas tribulações"!',
      phase_1: '⚡ "Perto está o Senhor dos que têm o coração quebrantado e salva os de espírito oprimido." (Salmo 34:18)\n\nNa depressão mais profunda, proteja sua ESPERANÇA, sua FÉ e seu AMOR!',
      phase_2: '🌩️ "Em tudo somos atribulados, porém não angustiados; perplexos, porém não desanimados." (2 Coríntios 4:8)\n\nOs relâmpagos do sofrimento não podem apagar a luz de Deus em você!',
      phase_3: '⛈️ "Tenho-vos dito estas coisas, para que tenhais paz em mim. No mundo, passais por aflições; mas tende bom ânimo; eu venci o mundo." (João 16:33)\n\nMesmo na tempestade mais escura, Cristo é a sua âncora!',
      defeated: '🌈 VITÓRIA SOBRE A TEMPESTADE!\n\n"Depois da chuva vem o sol. Ele dispersa as nuvens com sua luz." (Jó 37:11-15)\n\nVocê atravessou a tempestade! A esperança brilha novamente, mais forte do que antes.',
      wisdom: 'As depressões, as perdas graves e os medos repentinos são como tempestades que escurecem nosso céu interior. Mas a Bíblia nos lembra que "Aquele que habita no esconderijo do Altíssimo descansará à sombra do Onipotente." Nos momentos mais sombrios, a esperança, a fé e o amor são luzes que devemos proteger. Deus nunca abandona seus filhos na tempestade - Ele é nosso refúgio, nossa força, e após cada tempestade, Ele faz brilhar Seu arco-íris de promessas.'
    },
    black_raven: {
      name: '🐦‍⬛ O Corvo das Mentiras',
      intro: 'Um corvo negro cospe mentiras sobre sua identidade... Mas "és precioso aos meus olhos, és digno de honra, e eu te amo"!',
      phase_1: '🪶 "Eu te louvo, porque de um modo terrível e tão maravilhoso fui formado; maravilhosas são as tuas obras." (Salmo 139:14)\n\nAs mentiras dizem: NÃO BONITO O SUFICIENTE, NÃO RICO O SUFICIENTE, NÃO NORMAL...\nMas a VERDADE de Deus diz: VOCÊ É PRECIOSO!',
      phase_2: '✝️ "Vede que grande amor nos tem concedido o Pai, a ponto de sermos chamados filhos de Deus; e, de fato, somos filhos de Deus." (1 João 3:1)\n\nAs mentiras gritam: SEM VALOR, MUITO DIFERENTE, MUITO PECADOR...\nMas Deus diz: VOCÊ É MEU FILHO AMADO!',
      phase_3: '👑 "Vós, porém, sois raça eleita, sacerdócio real, nação santa, povo de propriedade exclusiva de Deus." (1 Pedro 2:9)\n\nAs mentiras berram: MUITO MAU, INDIGNO, REJEITADO...\nMas Cristo proclama: VOCÊ É ESCOLHIDO, VOCÊ É REAL, VOCÊ É AMADO!',
      defeated: '🌟 VITÓRIA SOBRE AS MENTIRAS!\n\n"Conhecereis a verdade, e a verdade vos libertará." (João 8:32)\n\nVocê conquistou as mentiras! Agora você conhece sua verdadeira identidade em Cristo.',
      wisdom: 'O corvo das mentiras representa todas as acusações e falsas crenças que o inimigo usa para destruir nossa identidade: "não bonito o suficiente", "não rico o suficiente", "muito diferente", "muito pecador". Essas mentiras são flechas envenenadas direcionadas aos nossos corações. Mas a Bíblia nos revela a VERDADE: fomos criados à imagem de Deus, redimidos pelo sangue de Cristo, adotados como filhos de Deus. Nosso valor não depende de nossa aparência, riqueza ou erros - vem do amor incondicional de Deus. Em Cristo, somos preciosos, amados, escolhidos, reais. Esta é a nossa verdadeira identidade!'
    }
  }
};

// Export
if (typeof module !== 'undefined' && module.exports) {
  module.exports = pt;
}
