// ============================================
// 🇵🇱 POLSKI - DOVE OF HOPE
// ============================================

const pl = {
  code: 'pl',
  name: 'Polski',
  flag: '🇵🇱',
  
  ui: {
    score: 'Wynik',
    clouds: 'Chmury',
    level: 'Rozdział',
    lives: 'Życie',
    language: 'Język',
    choose_language: 'Wybierz język',
    language_changed: 'Język zmieniony'
  },
  
  buttons: {
    start: 'Zapal!',
    play: 'GRAJ',
    restart: 'ZAGRAJ PONOWNIE',
    infinite_mode: 'Tryb Nieskończony',
    close: 'Zamknij',
    start_bonus: 'Rozpocznij'
  },
  
  messages: {
    start: 'Leć, mała gołąbko! Rozprosz chmury!',
    restart: 'Gołąbek odradza się! Ruszamy!',
    gameOver: 'Koniec gry! Ale w przeciwnościach błyszczy jeszcze jaśniej!',
    victory: 'ZWYCIĘSTWO! Psalm 99,1 - Życie to niezwykła przygoda! - mówi Początek',
    levelUp: 'Poziom {level} - Światło rośnie!',
    motivation: 'Czas zapalić światło - To dobry czas, aby spełnić swoje marzenia!',
    welcome: 'Witaj, Kadet Pierwszego Lotu!'
  },

  profile: {
    nickname: 'Twój pseudonim',
    email: 'Email (opcjonalny)',
    enter_nickname: 'Wprowadź swój pseudonim...',
    enter_email: 'twoj@email.pl'
  },
  
  gameplay: [
    'Ruszaj się, to czas!',
    'Gołąbek przynosi nadzieję!',
    'Kontynuuj, światło wraca!',
    'Twoje skrzydła niosą nadzieję!',
    'Wytrwaj, słońce na ciebie czeka!',
    'Leć ku swoim marzeniom!',
    'Po deszczu przychodzi słońce!',
    'Umieć tańczyć w deszczu to umieć być szczęśliwym!',
    'Twoja wiara jest silniejsza niż burza!',
    'Tańcz z żywiołami, jesteś wolny!'
  ],
  
  rain: [
    'Po deszczu przychodzi słońce!',
    'Twoja wiara jest silniejsza niż burza!',
    'Burza ujawnia siłę twoich skrzydeł!'
  ],
  
  powerups: {
    umbrella: 'Parasol aktywowany! Tańcz z żywiołami!',
    speed: 'Boska prędkość! Twoje skrzydła są wolne!',
    multishot: 'Potrójne światło! Twoje serce promieniuje!',
    rainbow: 'Tarcza Tęczowa aktywowana! Boska ochrona!',
    star_collected: '⭐ Gwiazdka zebrana! ({count} w zapasie) - Podwójne kliknięcie na falę uderzeniową!',
    level_up: '✨ Poziom {level}: {title}',
    new_record: '🏆 NOWY REKORD! Wynik: {score}',
    new_record_popup: '🎉 Nowy rekord!\n\nWynik: {score}\n\nTwój wynik został zapisany.\nKliknij Eksportuj w menu Skarb, aby się nim podzielić.',
    infinite_mode: '🔄 Tryb niekonczący się włączony! Wersety co 100 chmur ☁️',
    record_saved: '☁️ Rekord zapisany!',
    heart: '💖 +1 Życie! Łaska Boża chroni cię!',
    try_again: 'Ale w przeciwnościach błyszczy jeszcze jaśniej!',
    boss_defeated: '🏆 Boss pokonany! +{score} pkt',
    storm_cleared: '🌈 Burza się rozwiała! Nadzieja znów świeci!',
    continue: '✨ Kontynuuj',
    save_system: '✨ System zapisu'
  },
  
  bonus: {
    collect_blessings: 'Zbieraj błogosławieństwa:',
    star: 'Gwiazda',
    heart: 'Serce',
    cross: 'Krzyż',
    rainbow: 'Tęcza',
    coin_trap: 'Moneta = -200 pkt (PUŁAPKA!)',
    objective: 'Cel:',
    objects: 'przedmioty',
    level_title: 'POZIOM BONUSOWY'
  },
  
  treasure: {
    title: '💎 Menu Skarbów',
    my_records: '🏆 Moje Rekordy',
    best_score: 'Najlepszy Wynik',
    max_level: 'Maksymalny Poziom',
    star_stock: '⭐ Zapas Gwiazd',
    chapters_conquered: '🏅 Podbite Rozdziały',
    chapters_completed: 'rozdziały ukończone',
    infinite_progress: '♾️ Postęp Trybu Nieskończonego',
    verse_collection: 'Zbieranie losowych wersetów co 100 chmur',
    xp_clouds: 'XP: {count} chmur',
    next_verse: 'Następny werset za: {count}',
    verses_collected: '📚 {count} wersetów zebranych',
    last_played: 'Ostatnia gra',
    creator: '✨ Twórca',
    creator_name: '🎮 Emmanuel Payet',
    creator_bio: 'Deweloper & Twórca inspirujących gier',
    audiomack: '📱 Audiomack',
    creator_message: 'Niech ta gra cię inspiruje i wzmacnia! 🕊️',
    discover_artist: '🎵 Odkryj Artystę',
    cloud_connect: '☁️ Połącz się z chmurą',
    cloud_manage: '☁️ Zarządzaj moim połączeniem',
    life_tips_faith: '✝️ Wiara',
    life_tips_courage: '💪 Odwaga',
    life_tips_peace: '🕊️ Pokój',
    life_tips_love: '❤️ Miłość',
    life_tips_prompt: 'Kliknij zakładkę, aby odkryć radę życiową',
    infinite_chapter: 'Rozdział {number}',
    my_profile: 'Mój Profil',
    nickname: 'Pseudonim',
    email: 'E-mail',
    cloud_status: 'Chmura',
    connected: 'Połączony',
    disconnected: 'Rozłączony',
    highest_level: 'Najlepszy Wynik',
    last_game: 'Ostatnia gra',
    testimonies_title: 'Świadectwa Nadziei',
    testimonies_description: 'Odkryj historie odwagi i uzdrowienia',
    read_testimonies: 'Czytaj świadectwa',
    save_cloud: 'Zapisz w Chmurze',
    share_score: 'Udostępnij mój wynik',
    not_set: 'Nie ustawiono',
    boss_wisdom_title: 'Mądrość Bossów',
    boss_wisdom_subtitle: 'Pokonaj bossów, aby odblokować ich głębokie nauki',
    boss_wisdom_collected: 'mądrość odblokowana',
    boss_wisdom_locked: 'Ta mądrość jest jeszcze zablokowana. Pokonaj tego bossa, aby ją odblokować!',
    boss_wisdom_philosophy: 'Filozofia i Nauka',
    boss_wisdom_victory: 'Wiadomość o Zwycięstwie',
    boss_wisdom_close: 'Zamknij'
  },
  
  intro: 'Życie jest często długą bitwą, potrzeba odwagi i siły, aby przetrwać złe dni. Dzielny bohaterze, powstań i miłością Bożą przemień szare chmury w dobro.',
  
  clouds_cleared: '{count} chmur rozwianych',
  transform_clouds: 'Przemień szare chmury',
  

  
  levels: {
    1: 'Postawiłeś pierwsze kroki w światło! Nie jesteś sam w tej walce.',
    2: 'Jak orzeł, Bóg cię wzmacnia! Wybiera cię takim, jakim jesteś, z twoimi słabościami i pięknem.',
    3: 'Nie jesteś bezwartościowy, nie jesteś odrzucony. Jesteś nieskończenie kochany!',
    4: 'Twoje błędy cię nie definiują. Miłość Boga wymazuje wszystko i czyni cię wolnym!',
    5: 'Jesteś wybrany! Nie ze względu na twoją doskonałość, ale z czystej łaski.',
    6: 'W twojej słabości objawia się Jego siła. Możesz wszystko, bo On cię wzmacnia!',
    7: 'Wyzwania się mnożą, ale tańczysz z żywiołami!',
    8: 'Nie musisz być doskonały, by być kochanym. Jesteś już doskonale kochany!',
    9: 'Nie jesteś zbyt grzeszny dla Boga. Jesteś Jego bohaterem, Jego ukochanym dzieckiem!',
    10: 'Stare minęło, oto nowe! Odradzasz się w boskim świetle.',
    11: 'Powstań z odwagą! Jesteś godny, jesteś zdolny, jesteś zwycięski!',
    12: 'Biblia to nie księga sądu, ale skarb miłości dla ciebie! Otwórz ją z ufnością.',
    13: 'Zbliżasz się do szczytu! Czytaj Biblię jako zwycięzca, nie jako potępiony. Jesteś wybrany i kochany!',
    14: 'Ostatnie wyzwanie przed całkowitym zwycięstwem! Biblia na ciebie czeka, jest napisana dla ciebie, bohaterze światła! Kontynuuj, jesteś prawie na szczycie!'
  },
  
  victory_message: 'ZWYCIĘSTWO! Przemieniłeś ciemność w światło! Teraz otwórz swoją Biblię z ufnością: jesteś wybrany, jesteś kochany, jesteś zwycięski! "Życie jest niezwykłą przygodą" - Psalm 99:1 mówi Orygenes. Niech pokój i miłość panują w twoim sercu!',
  
  infinite_messages: [
    'Kontynuuj swoją przygodę! Każda rozproszona chmura to zwycięstwo!',
    'Jesteś mistrzem światła! Kontynuuj swoją misję!',
    'Nieskończoność należy do ciebie! Niech twoja wiara rośnie jeszcze bardziej!',
    'Przekraczasz wszystkie granice! Brawo, wieczny bohaterze!',
    'Twoja wytrwałość jest godna podziwu! Świeć dalej!'
  ],

  popups: {
    not_connected: '⚠️ Nie połączony!\n\nPołącz się najpierw przez menu Skarbów.',
    no_data_to_save: '⚠️ Brak danych do zapisania\n\nZagraj najpierw, aby utworzyć dane!',
    saving: '⏳ Zapisywanie...',
    save_failed: '❌ Nie udało się zapisać!\n\n{error}',
    save_error: '❌ Błąd zapisywania!\n\n{error}',
    data_imported: '✅ Dane zaimportowane pomyślnie!',
    invalid_file: '❌ Nieprawidłowy plik',
    enter_nickname_email: '❌ Proszę wprowadzić pseudonim I email!',
    invalid_email: '❌ Nieprawidłowy email!',
    connected_local_saved: '✅ Połączono!\n\nDane lokalne zapisane w chmurze.',
    connected_cloud_restored: '✅ Połączono!\n\nDane z chmury przywrócone.',
    connected_success: '✅ Pomyślnie połączono!\n\nTwój postęp będzie automatycznie zapisywany online.',
    reset_confirm: '⚠️ Czy na pewno chcesz zresetować wszystkie dane?',
    data_conflict: '⚠️ Wykryto konflikt danych!\n\nDane lokalne: Wynik {localScore}, Poziom {localLevel}\nDane w chmurze: Wynik {cloudScore}, Poziom {cloudLevel}\n\nKtórą wersję zachować?',
    connection_error: '❌ Błąd połączenia:\n{error}'
  },

  testimony: {
    title: '💎 Skarb Życia - Emmanuel',
    paragraph1: 'Był czas, kiedy choroba strasznie mnie izolowała i ciemne chmury napaścię na moje życie. Próbowałem się poddać, ale przed tym wezwałem Boga! <strong>Tobie oddaję swoje życie, a nie podstępnym cieniom.</strong>',
    paragraph2: 'Jak modlitwa, moje wołanie zostało usłyszane i moje życie zmieniło się od dnia, w którym poznałem w swoim sercu, że <em>jestem kochany takim, jakim jestem</em>. Długa praca zaufania, odwagi i zmiany zaczęła się we mnie.',
    paragraph3: 'Nauczyłem się na błędach, nauczyłem się nie patrzeć zawsze wstecz, nie zapominając. Ale przede wszystkim nauczyłem się patrzeć w przyszłość z cierpliwością, nadzieją i radością.',
    paragraph4: 'Radość, którą znalazłem w chwale i uwielbieniu Boga Zbawiciela, tego, który wyciągnął do mnie rękę, gdy nikt nie mógł być przy mnie. Wyprowadził mnie z doliny śmierci i pokazał mi, co jest naprawdę ważne: <strong>proste, zwykłe życie</strong>.',
    paragraph5: 'Zobaczyłem poza moimi materialistycznymi pragnieniami, zobaczyłem, że muszę patrzeć na to, co mam, a nie na to, czego nie mam. I kochać to, kim jestem, takim, jakim chcę być. <em>Zobaczyłem nadzieję twarzą w twarz.</em>',
    paragraph6: 'Prosiłem i otrzymałem niewysłowione odpowiedzi, które wytworzyły we mnie niezachwianą wiarę. Poprzez trudności rosłem i nie żałuję, że wytrwałem, miałem nadzieję raz za razem, ponieważ <strong>każde wzniesienie kryło skarby życia</strong>, każda góra pokonana odwagą ruchu przyniosła lepszą przyszłość, wspomnienia niezwykłego życia, wspaniałe spotkania i miejsca.',
    final_message: '🕊️ Dlatego Jezus mówi ci: Nigdy się nie poddawaj, wytrwaj do niemożliwego! 🕊️'
  },

  life_tips: {
    faith: {
      title: '✝️ Rady dotyczące Wiary',
      tip1: '💫 "Wiara jest pewnością tego, czego się spodziewamy" - Hebrajczyków 11:1',
      tip2: '🙏 Módl się każdego dnia, nawet kilka minut wystarczy, aby wzmocnić twoją duszę',
      tip3: '📖 Czytaj Biblię regularnie, aby karmić swoją wiarę i znajdować odpowiedzi',
      tip4: '✨ Zaufaj Bogu w trudnych czasach, On ma dla ciebie plan',
      tip5: '🌟 Otaczaj się wierzącymi, którzy zachęcają cię w twojej wierze'
    },
    courage: {
      title: '💪 Rady dotyczące Odwagi',
      tip1: '🦁 "Bądź mocny i odważny, nie bój się" - Jozuego 1:9',
      tip2: '🔥 Staw czoła swoim lękom jeden po drugim, każde małe zwycięstwo się liczy',
      tip3: '🎯 Wyznacz jasne cele i idź ku nim krok po kroku',
      tip4: '👊 Nigdy się nie poddawaj, nawet gdy jest trudno - wytrwałość się opłaca',
      tip5: '⭐ Pamiętaj o swoich przeszłych sukcesach, aby znaleźć siłę do kontynuowania'
    },
    peace: {
      title: '🕊️ Rady dotyczące Pokoju',
      tip1: '🌿 "Pokój mój zostawiam wam, pokój mój daję wam" - Jana 14:27',
      tip2: '🧘 Poświęć czas każdego dnia na odpoczynek i medytację',
      tip3: '🌺 Przebacz tym, którzy cię zranili, uwolnij się od ciężaru urazy',
      tip4: '🌈 Szukaj pokoju w małych rzeczach: natura, muzyka, modlitwa',
      tip5: '💕 Otaczaj się pozytywnymi ludźmi, którzy przynoszą ci spokój'
    },
    love: {
      title: '❤️ Rady dotyczące Miłości',
      tip1: '💖 "Miłujcie się wzajemnie, jak Ja was umiłowałem" - Jana 13:34',
      tip2: '🤝 Okazuj życzliwość wszystkim, nawet tym, którzy są inni',
      tip3: '🎁 Dawaj, nie oczekując niczego w zamian, prawdziwa miłość jest bezinteresowna',
      tip4: '👂 Naprawdę słuchaj innych, bądź obecny dla tych, którzy cierpią',
      tip5: '✨ Zacznij od kochania siebie: jesteś cenny w oczach Boga'
    }
  },

  ranks: {
    1: {
      title: 'KADET - Pierwsze Loty',
      message: 'Twoja niebiańska podróż się rozpoczyna. Przejmij kontrolę i naucz się nawigować między chmurami. Każda misja przybliża cię do przeznaczenia.'
    },
    2: {
      title: 'PILOT UCZŃ - W Treningu',
      message: 'Szybko się rozwijasz! Twoje umiejętności rosną, pewność siebie wzmacnia się. Wznoś się dalej ku wyżynom.'
    },
    3: {
      title: 'LOTNIK - Chrzest Nieba',
      message: 'Twoje skrzydła rozwijają się z pewnością siebie! Opanujesz podstawy lotu i odkrywasz wolność niebios.'
    },
    4: {
      title: 'DOŚWIADCZONY PILOT - Wprawny Nawigator',
      message: 'Wiatry cię znają, chmury cię szanują! Nawigu jesz z łatwością przez niebiańskie prądy.'
    },
    5: {
      title: 'AS NIEBIOS - Skrzydlaty Wirtuoz',
      message: 'Tańczysz między chmurami jak mistrz! Twoje refleksy się wyostrzają, precyzja staje się legendą.'
    },
    6: {
      title: 'ELITARNA ESKADRA - Siły Powietrzne',
      message: 'Jesteś wśród najlepszych! Twoja wiedza inspiruje innych pilotów. Latasz z boską gracją.'
    },
    7: {
      title: 'DOWÓDCA - Niebiański Taktyk',
      message: 'Powierzono ci misje strategiczne! Dowodzisz niebem z mądrością i odwagą, prowadzony przez boskie światło.'
    },
    8: {
      title: 'STRATEG CHMUR - Mistrz Taktyki',
      message: 'Przewidujesz każdy ruch! Twoja inteligencja taktyczna i wizja strategiczna przekraczają zwyczajność.'
    },
    9: {
      title: 'WETERAN - Wykuty przez Burze',
      message: 'Przetrwałeś tysiąc burz! Każda blizna opowiada historię zwycięstwa. Jesteś żywą legendą niebios.'
    },
    10: {
      title: 'NIEBIAŃSKI BOHATER - Uznany Mistrz',
      message: 'Twoje wyczyny są opiewane we wszystkich królestwach! Uosabiasz odwagę i niezłomną determinację.'
    },
    11: {
      title: 'LEGENDA - Żyjący Mit',
      message: 'Twoje imię odbija się echem we wszystkich niebiosach! Kadeci marzą, by stać się takimi jak ty. Uosabiasz doskonałość i wdzięk.'
    },
    12: {
      title: 'WIECZNY MISTRZ - Nieśmiertelna Chwała',
      message: 'Twoja sława przekracza epoki! Twoje dziedzictwo zainspiruje przyszłe pokolenia niebiańskich pilotów.'
    },
    13: {
      title: 'MISTRZ PILOT - Powietrzna Symfonia',
      message: 'Nie latasz już, tworzysz niebiańską symfonię! Każdy ruch to perfekcja, każda misja to arcydzieło.'
    },
    14: {
      title: 'SKRZYDŁO NIEBA - Wieczny Strażnik',
      message: 'Przekroczyłeś śmiertelność! Niebiański strażnik, obrońca niebios, twój lot jest teraz wieczny. Nieskończoność należy do ciebie!'
    }
  },

  level_titles: {
    1: 'Świt Nadziei',
    2: 'Boskie Tchnienie',
    3: 'Skrzydła Wiary',
    4: 'Lot Ducha',
    5: 'Niebiańskie Powołanie',
    6: 'Siła z Wysoka',
    7: 'Po Burzy',
    8: 'Miłość Zwycięża',
    9: 'Droga Wiary',
    10: 'Nowe Stworzenie',
    11: 'Męstwo Wojownika',
    12: 'Boskie Światło',
    13: 'Wybrany Lud',
    14: 'Wieczna Przygoda'
  },
  
  verses: {
    level: {
      1: { text: 'Światło świeci w ciemności', reference: 'Jana 1:5' },
      2: { text: 'Duch Boży unosił się nad wodami', reference: 'Rodzaju 1:2' },
      3: { text: 'Ci, którzy oczekują Pana, odzyskają siłę, wzniosą się jak orły', reference: 'Izajasza 40:31' },
      4: { text: 'Gdzie jest Duch Pański, tam jest wolność', reference: '2 Koryntian 3:17' },
      5: { text: 'Wołałem cię po imieniu, jesteś mój', reference: 'Izajasza 43:1' },
      6: { text: 'Wszystko mogę w Tym, który mnie umacnia', reference: 'Filipian 4:13' },
      7: { text: 'Po deszczu przychodzi słońce', reference: 'Hioba 37:11' },
      8: { text: 'Miłość nigdy nie ustaje', reference: '1 Koryntian 13:8' },
      9: { text: 'Twoja wiara cię uzdrowiła', reference: 'Łukasza 7:50' },
      10: { text: 'Jeśli kto jest w Chrystusie, jest nowym stworzeniem', reference: '2 Koryntian 5:17' },
      11: { text: 'Bądź mocny i odważny', reference: 'Jozuego 1:9' },
      12: { text: 'Twoje słowo jest lampką dla mój stóp', reference: 'Psalm 119:105' },
      13: { text: 'Wy jesteście wybranym rodem', reference: '1 Piotra 2:9' },
      14: { text: 'Życie jest niezwykłą przygodą', reference: 'Psalm 99:1 (mówi Orygenes)' }
    },
    rank: {
      1: { text: 'Z daleka ukazał mi się Pan: "Miłością wieczną cię umiłowałem, dlatego zachowałem dla ciebie łaskę.', reference: 'Jeremiasza 31:3' },
      2: { text: 'On dodaje mocy zmęczonemu i wzmacnia siły słabego.', reference: 'Izajasza 40:29' },
      3: { text: 'Ci, którzy oczekują Pana, odzyskają siłę, wzniosą się jak orły', reference: 'Izajasza 40:31' },
      4: { text: 'Gdzie jest Duch Pański, tam jest wolność', reference: '2 Koryntian 3:17' },
      5: { text: 'Wołałem cię po imieniu, jesteś mój', reference: 'Izajasza 43:1' },
      6: { text: 'Wszystko mogę w Tym, który mnie umacnia', reference: 'Filipian 4:13' },
      7: { text: 'Bóg bowiem nie jest Bogiem zamieszania, ale pokoju', reference: '1 Koryntian 14:33' },
      8: { text: 'Nie bój się, bo jestem z tobą', reference: 'Izajasza 41:10' },
      9: { text: 'Bądź silny i męny', reference: 'Jozuego 1:9' },
      10: { text: 'Twoja wiara cię ocaliła', reference: 'Łukasza 7:50' },
      11: { text: 'Jeśli ktoś jest w Chrystusie, jest nowym stworzeniem', reference: '2 Koryntian 5:17' },
      12: { text: 'Wy zaś jesteście wybranym plemieniem, królewskim kapłaństwem', reference: '1 Piotra 2:9' },
      13: { text: 'Po deszczu przychodzi słońce', reference: 'Hioba 37:11' },
      14: { text: 'Miłość nigdy nie ustaje', reference: '1 Koryntian 13:8' }
    },
    infinite: [
      { text: 'Niech się nie trwozy serce wasze', reference: 'Jana 14:1' },
      { text: 'Jestem z wami przez wszystkie dni', reference: 'Mateusza 28:20' },
      { text: 'Pan jest moą światłością i zbawieniem', reference: 'Psalm 27:1' },
      { text: 'Wy jesteście światłością świata', reference: 'Mateusza 5:14' },
      { text: 'Miłość zakrywa mnogość grzechów', reference: '1 Piotra 4:8' },
      { text: 'Tak bowiem Bog umiłował świat', reference: 'Jana 3:16' },
      { text: 'Szukajcie najpierw Królestwa Bożego', reference: 'Mateusza 6:33' },
      { text: 'Sprawiedliwy z wiary żyć będzie', reference: 'Rzymian 1:17' },
      { text: 'Wszystko możliwe jest temu, kto wierzy', reference: 'Marka 9:23' },
      { text: 'Bóg jest miłością', reference: '1 Jana 4:8' },
      { text: 'Nic nie może nas oddzielić od miłości Bożej', reference: 'Rzymian 8:39' },
      { text: 'Postępujcie według Ducha', reference: 'Galacjan 5:16' },
      { text: 'Radujcie się zawsze w Panu', reference: 'Filipian 4:4' },
      { text: 'Pokój Boży, który przewyższa wszelkie pojmowanie, będzie strzeżł serc waszych', reference: 'Filipian 4:7' },
      { text: 'Nie lękaj się, bo Ja jestem z tobą', reference: 'Izajasza 41:10' },
      { text: 'Pan jest moją siłą i tarczą', reference: 'Psalm 28:7' },
      { text: 'Błogosławieni czystego serca', reference: 'Mateusza 5:8' },
      { text: 'Nadzieja nie zawodzi', reference: 'Rzymian 5:5' },
      { text: 'Wszystko czynimy w miłości', reference: '1 Koryntian 16:14' },
      { text: 'Łaska Pana niech będzie z wami', reference: '1 Koryntian 16:23' }
    ]
  },

  bosses: {
    spider: {
      name: '🕷️ Pająk Ciemności',
      intro: 'Gigantyczny pająk tka pajęczynę strachu... Ale "Bóg nie dał nam ducha bojaźni"!',
      phase_1: '🕸️ "Albowiem nie dał nam Bóg ducha bojaźni, ale mocy i miłości, i trzeźwego myślenia." (2 Tymoteusza 1:7)\n\nSTRACH PRZED BÓLEM, STRACH PRZED UKĄSZENIEM... Te strachy cię paraliżują!',
      phase_2: '✨ "Nie bój się, bo Ja jestem z tobą; nie lękaj się, bo Ja jestem twoim Bogiem." (Izajasza 41:10)\n\nNIELOGICZNE LĘKI i FOBIE nie mają mocy nad kroczącymi w wierze!',
      phase_3: '🛡️ "Choćbym chodził ciemną doliną, zła się nie ulęknę, bo Ty jesteś ze mną." (Psalm 23:4)\n\nStrach to MECHANIZM OBRONNY, ale nie powinien tobą WŁADAĆ!',
      defeated: '🦋 ZWYCIĘSTWO NAD STRACHEM!\n\n"W miłości nie ma bojaźni, lecz doskonała miłość usuwa bojaźń." (1 Jana 4:18)\n\nPokonałeś paraliżujący strach! Wiara zamienia strach w odwagę.',
      wisdom: 'Pająk symbolizuje wszystkie nasze lęki: strach przed bólem, przed ukąszeniem, nielogiczne fobie. Te lęki są często normalnymi mechanizmami obronnymi, które chronią nas przed niebezpieczeństwem. Ale gdy stają się nadmierne, nas paraliżują. Biblia mówi: "Bóg nie dał nam ducha bojaźni, ale mocy, miłości i trzeźwego myślenia." Wiara nie przeczy niebezpieczeństwu, ale daje nam odwagę, by mu stawić czoła. W Chrystusie uczymy się odróżniać zdrową ostrożność od paraliżującego strachu. Doskonała miłość Boga usuwa bojaźń!'
    },
    flying_snake: {
      name: '🐍 Latający Wąż',
      intro: 'Starodawny wąż, symbol pokusy, podnosi się przed tobą... Ale "przeciwstawcie się diabłu, a ucieknie od was"!',
      phase_1: '🔥 "Czuwajcie i módlcie się, abyście nie popadli w pokusę." (Mateusza 26:41)\n\nPOKUSA, PIERWOTNE KŁAMSTWO, WĄTPLIWOŚĆ szepcze: "Czy rzeczywiście Bóg powiedział...?"',
      phase_2: '⚔️ "Przeciwstawcie się diabłu, a ucieknie od was. Zbliżcie się do Boga, a zbliży się do was." (Jakuba 4:7-8)\n\nJad KOMPROMISU i NIEPOSŁUSZEŃSTWA nie może dosięgnąć tych, którzy pozostają blisko Boga!',
      phase_3: '✝️ "A jak Mojżesz wywyższył węża na pustyni, tak musi być wywyższony Syn Człowieczy." (Jana 3:14)\n\nChrystus ZMIAŻDŻYŁ głowę węża! Jego zwycięstwo jest twoim zwycięstwem!',
      defeated: '🌟 ZWYCIĘSTWO NAD POKUSĄ!\n\n"Ono zmiażdży ci głowę, a ty ukąsisz je w piętę." (Rodzaju 3:15)\n\nPokonałeś pokusę! W Chrystusie starodawny wąż został pokonany.',
      wisdom: 'Latający wąż reprezentuje pokusę i pierwotne kłamstwo z Ogrodu Eden: "Na pewno nie umrzecie... będziecie jak Bóg." To głos, który sieje wątpliwość w dobroć Boga, pcha nas ku kompromisowi i nieposłuszeństwu. Ale Biblia uczy: "Przeciwstawcie się diabłu, a ucieknie." Pokusa nie jest grzechem - to zaproszenie do grzechu. Sam Jezus był kuszony, ale opierał się przez Słowo Boże. Na Krzyżu Chrystus zmiażdżył głowę węża. Przez Jego zwycięstwo mamy moc opierać się i pokonywać każdą pokusę. Zbliżcie się do Boga, a On zbliży się do nas!'
    },
    crazy_chicken: {
      name: '🐔 Szalony Kurczak',
      intro: 'Spanikowany kurczak biega na wszystkie strony... Fałszywe lęki doprowadzają cię do szaleństwa i paraliżują!',
      phase_1: '😰 "Albowiem nie dał nam Bóg ducha bojaźni, ale mocy i miłości, i trzeźwego myślenia." (2 Tymoteusza 1:7)\n\nFAŁSZYWE SUBIEKTYWNE LĘKI: "A co jeśli...?", "Może...", "Boję się, że..." - WYIMAGINOWANE lęki, które cię POWSTRZYMUJĄ!',
      phase_2: '🧠 "Nie martwcie się o nic, ale w każdej sprawie w modlitwie i błaganiach z dziękczynieniem zanoście swoje prośby do Boga." (Filipian 4:6)\n\nKATASTROFICZNE SCENARIUSZE w głowie, IRRACJONALNE LĘKI doprowadzające do szaleństwa! STOP!',
      phase_3: '✨ "Albowiem to Bóg sprawia w was i chcenie, i wykonanie według swego upodobania." (Filipian 2:13)\n\nTe SUBIEKTYWNE lęki to NIE RZECZYWISTOŚĆ! Bóg dał ci MĄDROŚĆ, nie SZALEŃSTWO!',
      defeated: '🦅 ZWYCIĘSTWO NAD FAŁSZYWYMI LĘKAMI!\n\n"Nie bój się, bo Ja jestem z tobą." (Izajasza 41:10)\n\nPokonałeś wyimaginowane lęki! Teraz kroczysz w mądrości, nie w szaleństwie.',
      wisdom: 'Szalony kurczak reprezentuje fałszywe subiektywne lęki, które doprowadzają nas do szaleństwa: "A co jeśli zawiodę?", "A co jeśli ludzie mnie osądzą?", "A co jeśli nie jestem wystarczająco dobry?". Te wyimaginowane lęki, te katastroficzne scenariusze w naszych głowach nas paraliżują i powstrzymują. Panikujemy jak przestraszony kurczak z powodu rzeczy, które prawdopodobnie nigdy się nie wydarzą! Biblia mówi: "Bóg nie dał nam ducha bojaźni, ale mocy, miłości i trzeźwego myślenia." 95% naszych lęków nigdy się nie spełnia. Te subiektywne lęki to nie rzeczywistość - to kłamstwa wroga, by zatrzymać nasz postęp. "Nie martwcie się o nic" - Bóg wzywa nas, by zastąpić fałszywe lęki wiarą, panikę pokojem, katastroficzne scenariusze zaufaniem do Jego opatrzności.'
    },
    grim_reaper: {
      name: '💀 Żniwiarz',
      intro: 'Sama śmierć stoi przed tobą... Ale pamiętaj: Chrystus zwyciężył śmierć!',
      phase_1: '🕊️ "Gdzież jest, o śmierci, twoje zwycięstwo? Gdzież jest, o śmierci, twój oścień?" (1 Koryntian 15:55)\n\nStrach przed śmiercią nie ma mocy nad wierzącymi w życie wieczne!',
      phase_2: '✝️ "Rzekł jej Jezus: Ja jestem zmartwychwstaniem i życiem. Kto we Mnie wierzy, choćby i umarł, żyć będzie." (Jana 11:25)\n\nŻycie zawsze triumfuje nad ciemnością! Każda chwila to ZWYCIĘSTWO!',
      phase_3: '🌟 "Naucz nas rachować nasze dni, abyśmy uzyskali mądre serce." (Psalm 90:12)\n\nNiezależnie od tego, ile czasu pozostało - ŻYJ PEŁNIĄ! Każdy dzień to DAR!',
      defeated: '🙏 ZWYCIĘSTWO NAD ŚMIERCIĄ!\n\n"Nie ma więc teraz żadnego potępienia dla tych, którzy są w Chrystusie Jezusie." (Rzymian 8:1)\n\nPokonałeś strach! Życie wieczne jest twoje przez łaskę Bożą.',
      wisdom: 'Śmierć nie jest końcem, ale przemianą. Przez Chrystusa przechodzimy z życia ziemskiego do życia wiecznego. Nieważne, ile czasu nam zostało lub jak żyjemy - najlepiej jest żyć każdą chwilę jako zwycięstwo, każdy rok jako dar. Nawet w chorobie, nawet w próbach, NADZIEJA i OPTYMIZM dają nam skrzydła i sprawiają, że niemożliwe często staje się możliwe! Biblia mówi: "Naucz nas rachować nasze dni, abyśmy uzyskali mądre serce." Każdy dzień jest cenny. Każdy oddech to łaska. Wiara chrześcijańska uczy nas, że śmierć została pokonana na Krzyżu, a nadzieja zmartwychwstania oświetla naszą drogę. Dlatego żyjmy pełnią życia, z radością i wdzięcznością, bo "To jest dzień, który uczynił Pan, radujmy się i weselmy w nim!" (Psalm 118:24)'
    },
    wall: {
      name: '🧱 Mur Łańcuchów',
      intro: 'Mur strachu i wątpliwości stoi przed tobą... Ale "gdzie jest Duch Pański, tam jest wolność"!',
      phase_1: '⛓️ "Do wolności wyzwolił nas Chrystus. Stójcie więc mocno i nie dajcie się znowu zaprzęgać w jarzmo niewoli." (Galatów 5:1)\n\nŁańcuchy STRACHU, WĄTPLIWOŚCI i WSTYDU nie mają nad tobą mocy!',
      phase_2: '🔓 "Jeśli więc Syn was wyzwoli, prawdziwie wolni będziecie." (Jana 8:36)\n\nKażdy zerwany łańcuch to zwycięstwo nad kłamstwami wroga!',
      phase_3: '✨ "Wszystko mogę w tym, który mnie wzmacnia." (Filipian 4:13)\n\nPrawda Boża burzy twierdze i mury, które cię więzią!',
      defeated: '🎉 WOLNOŚĆ W CHRYSTUSIE!\n\n"A poznacie prawdę, a prawda was wyzwoli." (Jana 8:32)\n\nZerwałeś łańcuchy! Jesteś wolny mocą miłości Bożej.',
      wisdom: 'Lęki są jak niewidzialne łańcuchy, które powstrzymują nas przed postępem. Biblia uczy nas: "Bóg nie dał nam ducha bojaźni, ale mocy, miłości i trzeźwego myślenia." Każdy lęk pokonany przez wiarę to zerwany łańcuch. W Chrystusie jesteśmy wezwani do życia w wolności, uwolnieni od kłamstw, które nas zniewolą. Prawda nas wyzwala!'
    },
    dark_cloud: {
      name: '☁️ Ciemna Chmura',
      intro: 'Burzowa chmura grozi pochłonięciem wszystkiego... Ale "Bóg jest dla nas ucieczką i siłą, pomocą w utrapieniach"!',
      phase_1: '⚡ "Pan jest bliski tym, którzy mają złamane serce, i zbawia skruszonych na duchu." (Psalm 34:18)\n\nW najgłębszej depresji chroń swoją NADZIEJĘ, WIARĘ i MIŁOŚĆ!',
      phase_2: '🌩️ "Zewsząd uciskani, ale nie pozbawieni wyjścia; zakłopotani, ale nie zrozpaczeni." (2 Koryntian 4:8)\n\nBłyskawice cierpienia nie mogą zgasić światła Bożego w tobie!',
      phase_3: '⛈️ "To wam powiedziałem, abyście we Mnie mieli pokój. Na świecie doznajecie ucisku, ale miejcie odwagę: Ja zwyciężyłem świat." (Jana 16:33)\n\nNawet w najciemniejszej burzy Chrystus jest twoją kotwicą!',
      defeated: '🌈 ZWYCIĘSTWO NAD BURZĄ!\n\n"Po deszczu przychodzi słońce. Rozprasza chmury swoim światłem." (Hioba 37:11-15)\n\nPrzetrwałeś burzę! Nadzieja znów świeci, mocniejsza niż przedtem.',
      wisdom: 'Depresje, ciężkie straty i nagłe lęki są jak burze, które zaciemniają nasze wewnętrzne niebo. Ale Biblia przypomina nam: "Kto mieszka w ukryciu Najwyższego, w cieniu Wszechmocnego przebywa." W najciemniejszych chwilach nadzieja, wiara i miłość to światła, które musimy chronić. Bóg nigdy nie opuszcza swoich dzieci w burzy - On jest naszą ucieczką, naszą siłą, a po każdej burzy sprawia, że błyszczy tęcza Jego obietnic.'
    },
    black_raven: {
      name: '🐦‍⬛ Kruk Kłamstw',
      intro: 'Czarny kruk wyrzuca kłamstwa o twojej tożsamości... Ale "jesteś drogi w moich oczach, cenny, i Ja cię umiłowałem"!',
      phase_1: '🪶 "Wysławiam cię, żem w sposób zadziwiający cudownie stworzony został. Cudowne są twoje dzieła." (Psalm 139:14)\n\nKłamstwo mówi: NIEWYSTARCZAJĄCO PIĘKNY, NIEWYSTARCZAJĄCO BOGATY, NIENORMALNY...\nAle PRAWDA Boża mówi: JESTEŚ CENNY!',
      phase_2: '✝️ "Patrzcie, jaką miłością obdarzył nas Ojciec, abyśmy byli nazwani dziećmi Bożymi, i nimi jesteśmy." (1 Jana 3:1)\n\nKłamstwo krzyczy: BEZWARTOŚCIOWY, ZBYT INNY, ZBYT GRZESZNY...\nAle Bóg mówi: JESTEŚ MOIM UKOCHANYM DZIECKIEM!',
      phase_3: '👑 "Ale wy jesteście wybranym plemieniem, królewskim kapłaństwem, narodem świętym, ludem nabytym." (1 Piotra 2:9)\n\nKłamstwo wyje: ZBYT ZŁY, NIEGODNY, ODRZUCONY...\nAle Chrystus ogłasza: JESTEŚ WYBRANY, JESTEŚ KRÓLEWSKI, JESTEŚ UKOCHANY!',
      defeated: '🌟 ZWYCIĘSTWO NAD KŁAMSTWAMI!\n\n"A poznacie prawdę, a prawda was wyzwoli." (Jana 8:32)\n\nPokonałeś kłamstwa! Teraz znasz swoją prawdziwą tożsamość w Chrystusie.',
      wisdom: 'Kruk kłamstw reprezentuje wszystkie oskarżenia i fałszywe przekonania, których wróg używa do zniszczenia naszej tożsamości: "niewystarczająco piękny", "niewystarczająco bogaty", "zbyt inny", "zbyt grzeszny". Te kłamstwa to zatrute strzały wymierzone w nasze serca. Ale Biblia objawia nam PRAWDĘ: jesteśmy stworzeni na obraz Boży, odkupieni krwią Chrystusa, adoptowani jako dzieci Boże. Nasza wartość nie zależy od wyglądu, bogactwa czy błędów - pochodzi z bezwarunkowej miłości Boga. W Chrystusie jesteśmy cenni, ukochani, wybrani, królewscy. To nasza prawdziwa tożsamość!'
    }
  }
};

// Export
if (typeof module !== 'undefined' && module.exports) {
  module.exports = pl;
}
